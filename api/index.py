...
# We put the UI directly inside the Python file to stop all 404 errors
DASHBOARD_HTML = """
<!DOCTYPE html>
<html>
<head>
                ...
                document.getElementById('results').innerHTML = data.map((item, i) => 
                    <div class="p-5 bg-zinc-900 rounded-xl border border-zinc-800 flex justify-between items-center">
                        <div>
                            ...
                            <p class="text-[9px] font-mono text-zinc-600">${item.ca}</p>
                        </div>
                        <div class="text-right">
                            ...
                            <a href="${item.buy.link}" target="_blank" class="bg-white text-black text-[10px] px-3 py-1 rounded font-black uppercase hover:bg-blue-500 hover:text-white">GO TO POOL</a>
                        </div>
                    </div>
                ).join('');
...
async def scan():
    # Only scanning major chains for maximum speed and stability
    ...
    for chain in chains:
        try:
            ...
            for pool in data:
                ...
                if not symbol: continueif symbol not in groups: groups[symbol] = []
